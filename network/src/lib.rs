...
mod tests {
    ...
    #[tokio::test]
    async fn test_swarm() {
        let mut client_config1 = development_net_conf(26117, false);
        let mut client_config2 = development_net_conf(26118, true);
        ...
        loop {
            select! {
                event1 = swarm1.select_next_some() => {
                    ...
                    match event1 {
                        event =>  DecentNetworkBehaviour::handle_swarm_event(&mut swarm1, event, &mut client_config1)
                    }
                },
                event2 = swarm2.select_next_some() => {
                    #[allow(clippy::match_single_binding)]
                    match event2 {
                        event =>  DecentNetworkBehaviour::handle_swarm_event(&mut swarm2, event, &mut client_config2)
                    }
                },
            };
        }
