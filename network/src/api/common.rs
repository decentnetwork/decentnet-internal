...
fn list_nodes(with_addresses: bool, config: &NetworkConfig) -> Vec<String> {
...
fn get_nodes_from_bootnodes() -> Result<bool, String> {
    let network_config = unsafe { NETWORK_CONFIG.as_ref().unwrap() };
    let swarm = unsafe { SWARM.as_mut().unwrap() };
    let runtime = unsafe { RUNTIME.as_mut() };
    if runtime.is_none() {
        return Err("Initialise Runtime before invoking dial()".to_string());
    ...
    runtime.unwrap().block_on(async {
        DecentNetworkBehaviour::get_nodes_from_bootnodes(swarm, network_config)
    });
    Ok(true)
}
